...
const ROLES = ["Full-Stack Developer", "React / Next.js Engineer", "AI-Integrated App Builder"];
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute bottom-[-20%] right-[-10%] w-[600px] h-[600px]
      ...
      >
        ...
        <motion.h1
          variants={itemVariants}
          className="font-display text-[clamp(44px,7.5vw,90px)] font-extrabold tracking-[-0.04em] text-slate-100 leading-[1.0] mb-4"
        >
          I Build Products<br />
        ...
        {/* Subtitle */}
        <motion.p variants={itemVariants} className="text-[16px] text-slate-500 leading-[1.85] max-w-[540px] mb-11">
          Full-stack engineer specialising in React, Next.js, Node.js, and Python AI integrations.
          I turn ideas into polished, production-grade products — fast, scalable, and built to last.
        </motion.p>
        ...
        <motion.div variants={itemVariants} className="flex flex-wrap gap-3 mb-16">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 text-slate-400 hover:text-sky-400 text-[14px] font-medium
              px-7 py-3.5 rounded-[9px] border border-white/10 hover:border-sky-400/50
        ...
        <motion.div variants={itemVariants} className="flex flex-wrap">
          {STATS.map((s, i) => (
            <div
              key={i}
              className={`px-8 py-4 border border-white/[0.07]
                ${i < STATS.length - 1 ? "border-r-0" : ""}
                ${i === 0 ? "rounded-l-xl" : ""}
                ${i === STATS.length - 1 ? "rounded-r-xl border-r border-white/[0.07]" : ""}`}
            >
              <div className="font-display text-[26px] font-extrabold text-slate-100 tracking-tight">{s.n}</div>
              <div className="text-[10px] font-semibold text-slate-600 uppercase tracking-[0.08em] mt-0.5">{s.l}</div>
            </div>
          ))}
        </motion.div>
      </motion.div>
